...
export class HomeComponent implements OnInit {
  ...
  addToCart(product: any) {
    if (!this.product_service.productInCart(product)) {    //if there a product in a cart
      product.quantity = 1;
      this.product_service.addToCart(product);
      this.products = [...this.product_service.getProduct()];  //get all product into new array
